    ...
    private static final String xyx_002 = "";
     private static final String
           ...
           " c.ZIP,c.EMAIL,c.user_pass,c.subject_text,c.subject_body,c.access_lvl,  s.sys_desc " +
            "      from mgn_user c ,mgn_lookup_sys s where  c.access_lvl = s.sys_id "
        
            ;
    public static final String
            select = sqlV2+" order by c.LAST_NAME,c.FIRST_NAME",
            select_byDept = sqlV2+" and c.user_id in (select x.user_id from mgn_user_dept x where c.user_id = x.user_id and dept_id = ?)",
            select_eMail = sqlV2+" and e c.EMAIL = ?",
